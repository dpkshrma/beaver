...
const TodoTimer = ({ timeTillDeadline, deactivateTodo }) => {
  ...
  const todoTimeOffset =
    timeTillDeadline > 0 ? `${todoTime} left` : `${todoTime} overdue`;
  return (
    ...
    <div className="todo-item padded">
      ...
      <div className="todo-item-content">
        <div className="todo-item-heading">
          <div className="todo-item-title" onClick={() => onTitleClick(_id)}>
            {title}
          </div>
          <div className="todo-item-actions">
            <i
              className="todo-item-action icon icon-pencil"
        ...
        <div className="todo-item-info">
          <div className="text-subtle">
            ...
            {status === 'inactive' ? (
              <TodoInactiveStatus activateTodo={activateTodo} />
            ) : null}
            {status === 'completed' ? (
              <TodoCompletedStatus completedAt={completedAt} />
            ) : null}
          </div>
          <div className="text-subtle">
